...
owner_id = bot_settings["bot settings"]["owner_id"]
...
async def give_voice_points(user_id):
    r = economy.find_one({"_id": user_id})
    ...
    if dm_or_not == "Dm":
        economy.update_one({"_id": user_id}, {"$set": {"wallet": user_balance + random_amount}})
        economy.update_one({"_id": user_id}, {"$set": {"voice_time": user_voice_time + 1}})
...
async def reset_reward():
    economy.update_many({"received": True}, {"$set": {"received": False}})
